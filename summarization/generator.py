...
def generate_mock_response(prompt: str) -> str:
    ...
    if "Patient Explanation Output:" in prompt or "simple, clear English" in prompt or "simple Grade 6 English" in prompt:
        # Dynamically extract key medical facts from clinical report or prescription prompt
        diag_match = re.search(r'(?:3\.\s+\*\*Diagnosis\*\*:\s*|DIAGNOSIS:\s*)(.*?)(?=\n\n|\n[4-9]\.|\n[A-Z]+:|\Z)', prompt, re.DOTALL | re.IGNORECASE)
        treat_match = re.search(r'(?:6\.\s+\*\*Treatment Given\*\*:\s*|TREATMENT:\s*|PHYSICIANS:\s*)(.*?)(?=\n\n|\n[7-9]\.|\n[A-Z]+:|\Z)', prompt, re.DOTALL | re.IGNORECASE)
        meds_match = re.search(r'(?:8\.\s+\*\*Discharge Medications\*\*:\s*|PRESCRIBED MEDICATIONS:\s*)(.*?)(?=\n\n|\n9\.|\nSAFETY|\nEXECUTIVE|\Z)', prompt, re.DOTALL | re.IGNORECASE)
        fup_match = re.search(r'(?:9\.\s+\*\*Follow-up\*\*:\s*|SAFETY PRECAUTIONS & ADVICE:\s*)(.*?)(?=\n\n|\nEXECUTIVE|\Z)', prompt, re.DOTALL | re.IGNORECASE)

        raw_diag = re.sub(r'\[\d+\]', '', diag_match.group(1)).strip() if diag_match else "your medical condition"
        raw_treat = re.sub(r'\[\d+\]', '', treat_match.group(1)).strip() if treat_match else "targeted medical care and supportive therapies"
        ...
        # Clean bullet point prefixes and extra whitespace
    ...
    # 2. Check if it's Doctor Summary Prompt (or initial prompt with context)
        ...
        # Helper to strip binary junk, recurring uppercase section headers and broken sentence fragments
        def sanitize_sentence(sent: str) -> str:
            clean = re.sub(r'[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x9F]', '', sent)
            clean = re.sub(r'[^\x20-\x7E\u0980-\u09FF\n\r\t•\-]', '', clean)
            clean = re.sub(r'^(CLINICAL DISCHARGE SUMMARY|PATIENT DEMOGRAPHICS|HISTORY OF PRESENT ILLNESS|CHIEF COMPLAINT|CURRENT MEDICATIONS & ALLERGIES|PHYSICAL EXAMINATION|HOSPITAL COURSE|DISCHARGE INSTRUCTIONS|LABORATORY DATA|GENERAL:)\s*', '', clean, flags=re.IGNORECASE)
